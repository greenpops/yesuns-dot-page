...
import { happinessAnthem, happyBirthday, kpopDemonHuntersGolden } from "@/lib/songs"
...
export default function Piano() {
  ...
  return (
    <div className="w-full flex flex-col items-center space-y-4">
      <div className="w-full flex flex-wrap justify-center text-gray-500">
        {octaves.map((octave) =>
          notes.map((note) => (
            <PianoKey
              key={`${note}${octave}`}
      ...
      <div className="flex space-x-4 flex-wrap justify-center text-gray-500">
        <Button onClick={() => playSong(kpopDemonHuntersGolden, 400)} disabled={isPlaying}>
          <Play className="mr-2 h-4 w-4" /> Play K-Pop Demon Hunters Golden
        </Button>
        <Button onClick={() => playSong(happinessAnthem, 400)} disabled={isPlaying}>
          <Play className="mr-2 h-4 w-4" /> Play Happiness Anthem
        </Button>
        <Button onClick={() => playSong(happyBirthday, 600)} disabled={isPlaying}>
          <Play className="mr-2 h-4 w-4" /> Play Happy Birthday
        </Button>
      </div>
      <motion.p
        initial={{ opacity: 0 }}
